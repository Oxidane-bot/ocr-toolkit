...
class QualityEvaluator:
    ...
    def get_file_type_preference(self, file_path: str) -> dict[str, float]:
        ...
        return {"markitdown_preference": markitdown_pref, "ocr_preference": ocr_pref}

    def compare_results(
        self, markitdown_result: dict[str, Any], ocr_result: dict[str, Any], file_path: str
    ) -> dict[str, Any]:
        """
        ...
        md_available = (
            markitdown_result
            and markitdown_result.get("success", False)
            and bool(markitdown_result.get("content"))
        )
        ocr_available = (
            ocr_result and ocr_result.get("success", False) and bool(ocr_result.get("content"))
        )
        ...
        if md_final_score > ocr_final_score:
            ...
            comparison["selection_reason"] = (
                f"MarkItDown scored {md_final_score:.1f} vs OCR {ocr_final_score:.1f} (+{score_diff:.1f})"
            )
        else:
            comparison["chosen_method"] = "ocr"
            score_diff = ocr_final_score - md_final_score
            comparison["selection_reason"] = (
                f"OCR scored {ocr_final_score:.1f} vs MarkItDown {md_final_score:.1f} (+{score_diff:.1f})"
            )
