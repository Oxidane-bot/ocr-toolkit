...
from ocr_toolkit.utils.file_discovery import discover_files, get_output_file_path, get_directory_cache
...
class TestPreserveStructureIntegration:
    ...
    def test_discover_files_with_real_nested_structure(self, nested_test_structure):
        ...
        assert len(files) == 7, f"Expected 7 files, found {len(files)}: {[Path(f).name for f in files]}"
    ...
    def test_output_path_generation_with_structure_preservation(self, nested_test_structure, temp_output_dir):
        ...
        for file_path in files:
            ...
            if "level" in file_path:
                assert "level" in output_path, f"Output path should preserve 'level' directories: {output_path}"
            
            if "alternative_branch" in file_path:
                assert "alternative_branch" in output_path, f"Output path should preserve alternative_branch: {output_path}"
        
        # All generated paths should be unique
        assert len(set(generated_paths)) == len(generated_paths), "All output paths should be unique"
    ...
    def test_non_recursive_vs_recursive_behavior(self, nested_test_structure):
        ...
        assert len(non_recursive_files) == 0, "Non-recursive search should find no files at root level"
    ...
    def test_error_handling_with_problematic_paths(self, temp_output_dir):
        ...
        assert discovery_time < 5.0, f"Discovery with edge case paths took too long: {discovery_time:.2f}s"
        
        # Should find all edge case files
        assert len(files) == len(edge_files), f"Should find all edge case files: {len(files)} != {len(edge_files)}"
    ...
    def test_symlink_handling_safety(self, temp_output_dir):
        ...
        try:
            ...
            if os.name != 'nt':  # Skip on Windows which has limited symlink support
                ...
                assert discovery_time < 10.0, f"Symlink cycle handling took too long: {discovery_time:.2f}s"
