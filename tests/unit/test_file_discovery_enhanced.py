...
class TestEnhancedFileDiscovery:
    ...
    def test_discover_files_recursive_default(self, tmp_path):
        ...
        assert (
            "subdir/file2.pdf" in relative_paths.values()
            or "subdir\\file2.pdf" in relative_paths.values()
        )
    ...
    def test_get_output_file_path_preserve_structure(self, tmp_path):
        ...
        output_path = get_output_file_path(
            input_path, output_dir, preserve_structure=True, relative_path=relative_path
        )

        # Normalize path separators for cross-platform compatibility
        output_path_normalized = output_path.replace("\\", "/")
        expected_path_normalized = str(
            tmp_path / "output" / "docs" / "section1" / "file.md"
        ).replace("\\", "/")
    ...
    def test_get_output_file_path_flat_structure(self, tmp_path):
        ...
        output_path = get_output_file_path(
            input_path, output_dir, preserve_structure=False, relative_path=relative_path
        )
    ...
    def test_discover_files_mixed_extensions(self, tmp_path):
        ...
        for file_path in files:
            ...
            assert ext in supported_exts, (
                f"File {file_path} with extension {ext} should not be found"
            )
    ...
    def test_discover_files_real_nested_structure(self):
        ...
        for file_path in files:
            ...
            assert not rel_path.startswith("/") and not rel_path.startswith("\\"), (
                f"Relative path should not be absolute: {rel_path}"
            )
            # Relative path should contain directory structure
            if "level" in file_path:
                assert "level" in rel_path, (
                    f"Relative path should preserve directory structure: {rel_path}"
                )
    ...
    def test_discover_files_real_nested_non_recursive(self):
        ...
        assert len(files) == 0, (
            f"Non-recursive search should find no files at root level, found: {files}"
        )
    ...
    def test_discover_files_boundary_conditions(self, tmp_path):
        ...
        assert len(levels_in_path) == 10, (
            f"All directory levels should be in relative path: {rel_path}"
        )
    ...
    def test_discover_files_many_files_per_level(self, tmp_path):
        ...
        for i in range(5):
            ...
            assert len(level_files) == 3, (
                f"Level {i} should have 3 files, found {len(level_files)}: {level_files}"
            )
    ...
    def test_get_output_file_path_deep_nested_structure(self, tmp_path):
        ...
        deep_input = (
            tmp_path / "docs" / "year2024" / "projects" / "section1" / "subsection" / "document.pdf"
        )
        ...
        output_path = get_output_file_path(
            str(deep_input), str(output_dir), preserve_structure=True, relative_path=relative_path
        )
        ...
        flat_output_path = get_output_file_path(
            str(deep_input), str(output_dir), preserve_structure=False, relative_path=relative_path
        )
